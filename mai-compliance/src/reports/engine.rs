...
fn render_text(payload: &ReportPayload) -> String {
    ...
    let _ = writeln!(
        s,
        "Island Mountain MAI — Lamprey Compliance Report"
    );
    let _ = writeln!(s, "================================================");
    let _ = writeln!(s, "Report type   : {}", payload.metadata.report_type.as_str());
    let _ = writeln!(
        s,
        "Date range    : {} → {}",
    ...
    let _ = writeln!(s, "-- Generated by Island Mountain MAI — Lamprey Compliance Layer --");
    s
}
...
fn render_html(payload: &ReportPayload) -> String {
    ...
    let _ = writeln!(s, "<h2>Summary</h2><p>{}</p>", html_escape(&payload.summary));
    let _ = writeln!(
        s,
