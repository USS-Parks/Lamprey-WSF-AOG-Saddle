...
**Current execution status (supersedes the cumulative roll-up below):** **AUTHORIZED FOR FULL STS EXECUTION — SAD-43 COMPLETE; SAD-44 NEXT.**
...
**Latest verified remote checkpoint:** SAD-43 final verification checkpoint `062b1e1592c5946a4fe884101fc7093146321df4` passed every applicable GitHub push workflow before this documentation closeout.

**SAD-HIST-04 reconciliation closeout:** **COMPLETE.** All 38 approved sanitized Mighty Eel history refs are published beneath immutable archive ruleset `19173522`. Reviewed implementation head `85b5c0925136ce7ea9865f197e1295db42ed07ca` passed the complete applicable GitHub workflow stack in pull request 4 and merged without squashing as `2937f6494561fc607519de6c17c259bc7c684e51`. No archive ref is a parent of active Saddle `main`, and the active dependency graph is unchanged. This bounded reconciliation closeout did not itself supersede or advance the canonical SAD implementation roster.
...
**SAD-34 closeout:** **COMPLETE; SAD-35 NEXT.** SAD-34 adds a non-forgeable `ActionGate` for exact model/tool/control actions. It binds receipt intent to the request digest, consumes a lineage-scoped nonce, reserves the runtime budget atomically across destinations, commits a metadata-only WSF-ledger authorization proof, then rechecks current revocation and action expiry immediately before invoking the private effect. The adversarial gate proves cross-tenant theft, replay, revocation/expiry races, budget races, receipt mismatch, and unavailable/empty receipt proof cannot reach an effect. Its implementation commit is `3d65900c870e10a812cc1468e3b005cfce96a931`, deterministic evidence is `test-evidence/saddle/SAD-34/action-reauthorization-gate.json`, and GitHub `main` was live-verified at the checkpoint above with the evidence blob matching local Git. The generic enforcement layer is complete; persisted typed `RuntimeGrant` handoff and composition of the real gateway/toolproxy/control consumers remain explicitly assigned to the SAD-35 live two-tenant gate.

**SAD-35 closeout:** **COMPLETE; SAD-40 NEXT.** SAD-35 persists and verifies the signed typed placement/runtime handoff, requires that handoff plus the existing child capability immediately before node process start, and composes the real AOG gateway and toolproxy effect seams plus the control action through a shared fail-closed `ActionSessionRegistry`. The live two-tenant gate uses OpenBao, redb close/reopen recovery, real ML-DSA and monotonic revocation state, the process driver, gateway provider, tool executor, shared budget accounting, and WSF-ledger receipts. It proves cross-tenant theft denial, tenant-local authority outage isolation and recovery, signed revocation denial across model/tool/control, continued service for the unaffected tenant, and serialized off-host receipt-chain verification. Its implementation commit is `ef8b27236e1e4f24b27fbad9b249703299291bfc`, deterministic evidence is `test-evidence/saddle/SAD-35/two-tenant-bridge-gate.json`, and all applicable GitHub push workflows passed before this closeout was recorded.

**SAD-40 closeout:** **COMPLETE; SAD-41 NEXT.** SAD-40 expands the typed declarative estate to 19 kinds with `ResourceQuota`, `PriorityClass`, `PlacementGroup`, `DisruptionBudget`, `RuntimeClass`, and `NodeLease`; applies fail-closed validation, estate-authority admission, optimistic concurrency, watches, two-phase finalization, and envelope sealing of runtime credential references; and adds a fail-closed `saddle.islandmountain.io/v1` conversion hub with exact legacy rollback. Its 512-case deterministic conversion fuzz gate preserves authority, UID, generation, resource version, desired state, and status, while integration tests exercise the real API and Raft store. The implementation commit is `99b31ebe9415cdd066c0d5fed559ffe9a7747465`; deterministic evidence is `test-evidence/saddle/SAD-40/declarative-estate-gate.json`; independence-evidence correction `a337ba3e46fd210a13f37f9bdde22eaf5099c45b` and active-name fixture correction `14672ead45b2f4effe65f42c27590cea1be0dcab` preserve the repository-wide gates. All applicable GitHub push workflows passed at the final implementation checkpoint before this closeout was recorded.

**SAD-41 closeout:** **COMPLETE; SAD-42 NEXT.** SAD-41 makes leadership authority contingent on current quorum confirmation, rejects authoritative writes through minority or stale leaders, preserves exact revisioned truth through versioned BLAKE3-checksummed snapshot installation, enforces known-member safe-count quorum-overlap changes, fences removed members, and converts lagged watches into a fail-closed relist at the exact current revision. Its five-case Jepsen-style gate exercises 6 concurrent clients with 32 attempts each across leader transition, exact late-learner snapshot restore, four-node membership rotation, 100 watch-buffer overflow events, invalid membership changes, and minority partition fencing with no stale authoritative allow. The implementation commit is `d3f26627b4912e8cd25c916dcbddfdf601a8af85`; final merged-boundary verification checkpoint is `ca6ee6ba21b59ce14f51d2f0f19f87a58ab55b66`; deterministic evidence is `test-evidence/saddle/SAD-41/consensus-truth-gate.json`. All applicable GitHub push workflows passed before this closeout was recorded.

**SAD-42 closeout:** **COMPLETE; SAD-43 NEXT.** SAD-42 makes reconciliation level-triggered from current state, converges across duplicate, reordered, and dropped events through full relist and periodic resync, applies deterministic jittered exponential backoff with bounded visible dead letters and redrive, cancels hung work at its deadline and in-flight work at shutdown, and preserves recoverable finalizer truth across restart while withdrawing external state before deletion. Its deterministic gate covers 256 fault histories, 86 overflow histories, 52 restart histories, and 100 watch-overflow events per injected history. The implementation commit is `870947c6c05329c47165eef7e4ae07fa6dd5eca9`; evidence-boundary repair is `0aeb54bfcb76cad7f110761629003faf915150a3`; initial Windows leader-selection repair is `c03fbfac09ee0be12512b70c8f8b6f921a6583bc`; mid-operation failover repair and final verified checkpoint is `f502b0881518b186a869b19f1271c41861419412`; deterministic evidence is `test-evidence/saddle/SAD-42/reconciliation-gate.json`. All applicable GitHub push workflows passed at the final checkpoint before this closeout was recorded.

**SAD-43 closeout:** **COMPLETE; SAD-44 NEXT.** SAD-43 adds the ordered QueueSort-through-PostBind plugin cycle over coherent revisioned snapshots; deny-wins sovereignty, resource, runtime, lease, and topology filters; weighted dominant-resource fairness with guarantees, priority, and bounded starvation; hard quotas; atomic reserve/unreserve and CAS binding; permit waits; all-or-nothing gang placement; deterministic disruption-aware multi-victim preemption; spread and accelerator-locality scoring; and authoritative budget/ROI inputs. Its 10-case deterministic gate covers 256 adversarial multi-tenant histories with 64 operations each and proves no sovereignty bypass, double bind, quota or capacity oversubscription, reservation leak, or feasible-work starvation. The implementation commit is `2a2dd39959654ea159056f94573e13ddad893e22`; tracked-evidence repair is `fe615e1ab6b61f6e2658510d8c5d3269263a4b45`; cross-platform evidence normalization and final verified checkpoint is `062b1e1592c5946a4fe884101fc7093146321df4`; deterministic evidence is `test-evidence/saddle/SAD-43/professional-scheduler-gate.json`. All applicable GitHub push workflows passed at the final checkpoint before this closeout was recorded.
...
- [x] **SAD-35 — Live two-tenant bridge gate.** Exercise two tenants through live OpenBao, Saddle control plane/nodes, AOG gateway/toolproxy, revocation, policy, restart, and network transition.
...
- [x] **SAD-40 — Declarative estate and conversion.** Finalize the imported kinds plus `ResourceQuota`, `PriorityClass`, `PlacementGroup`, `DisruptionBudget`, `RuntimeClass`, and `NodeLease`; implement optimistic concurrency, watches, finalizers, sealed sensitive fields, and `saddle.islandmountain.io/v1` conversion.
...
- [x] **SAD-41 — Consensus truth and fencing.** Prove linearizable writes, leader transitions, snapshots, membership change, bounded-stale watches, and minority fencing.
  **Gate:** Jepsen-style concurrency and partition tests show no stale authoritative allow.

- [x] **SAD-42 — Level-triggered reconciliation.** Prove idempotency, replay, duplicate/drop tolerance, backoff, resync, finalization, and convergence.
  **Gate:** fault-injected histories converge to the same state.

- [x] **SAD-43 — Professional scheduler framework and fairness.** Implement the ordered queue-sort through post-bind plugin cycle, coherent snapshots, hard trust/resource/topology filters, weighted dominant-resource fairness, quotas, priority, atomic reserve/unreserve, permit, gang scheduling, deterministic binding, preemption, spread, accelerator topology/locality, budget/ROI scoring, and starvation resistance.
