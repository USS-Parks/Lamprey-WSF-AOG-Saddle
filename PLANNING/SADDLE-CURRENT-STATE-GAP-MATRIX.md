...
| WSF-AOG bridge | Partial | SAD-30 froze the non-deserializable request/admission/placement/runtime/action contracts; SAD-31 consumes request/admission grants; SAD-32 hardens placement feasibility. SAD-33 adds a signed, placement/node/workload-digest-specific child capability and verifies it at the node's last responsible moment, with fixed per-kind AOG roles, budget/TTL/lineage attenuation, and revocation. | Bind the typed `PlacementGrant`/`RuntimeGrant` proof values themselves into the serialized handoff, then add per-action enforcement and the live two-tenant gate in SAD-34/SAD-35. |
...
| Fail-closed behavior | Partial | SAD-33 removes the unrestricted system-admission seam from release builds and denies missing/cross-tenant capability roots, expired/revoked/out-of-scope controller grants, sibling runtime tokens, changed digests, and revoked children. | Complete typed runtime/action grant consumption and prove unavailable trust state across the live two-tenant path. |
