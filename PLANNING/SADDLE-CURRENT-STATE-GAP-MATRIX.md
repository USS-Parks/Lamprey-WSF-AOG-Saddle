...
| Saddle identity | Partial | SAD-20 moved the canonical 12-package set and three binaries; SAD-21 moved active protocol, trust, persistence, API, environment, deployment, fixture, CI, and operator-document identities with deterministic evidence and no runtime aliases. Versioned legacy-state conversion and the final repository-wide eradication gate remain. | Complete SAD-22 inspect/dry-run/apply/verify/rollback migration and SAD-23 active-name eradication without restoring legacy authority. |
...
3. **Complete the Saddle rename.** SAD-20 completed the 12-package and three-binary cutover; SAD-21 completed the active runtime, protocol, trust, persistence, deployment, API-group, finalizer, label, SPIFFE, fixture, CI, and operator-document cutover. SAD-22 and SAD-23 still own versioned legacy-state conversion and the final active-name eradication gate. Leave Loom only in explicit historical migration notes and bounded migration fixtures.
