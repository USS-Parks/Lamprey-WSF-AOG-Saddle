...
| WSF-AOG bridge | Partial | SAD-30 adds explicit `saddle-bridge` and the non-deserializable `VerifiedSaddleRequest` → admission → placement → runtime → action grant contracts with WSF verification/revocation reuse, AOG deny-wins reuse, durable replay adapter, and property evidence. | Wire the frozen contracts through real admission, scheduling, node, and AOG action paths in SAD-31 through SAD-35. |
...
4. **Create the authority bridge.** SAD-30 completed the explicit typed contract and compatibility/property gate. SAD-31 through SAD-35 must wire controller identities, durable grant/audit linkage, real runtime/action enforcement, and remove production system bypasses.
