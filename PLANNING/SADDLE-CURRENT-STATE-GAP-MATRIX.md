...
| WSF-AOG bridge | Present | SAD-30 through SAD-34 establish the non-forgeable grant chain and proof-before-effect `ActionGate`. SAD-35 signs, persists, reloads, and verifies the typed placement/runtime handoff; requires it with the signed child capability at node process start; and composes managed gateway, toolproxy, and control effects through tenant-scoped action sessions with shared budget and WSF-ledger receipts. The live two-tenant gate proves restart recovery, cross-tenant denial, tenant-local authority-loss isolation, signed revocation, and off-host receipt verification. | Preserve this bridge contract while later prompts expand resource kinds, controller profiles, runtime drivers, scale, federation, and release profiles. |
...
| Scheduler | Present | SAD-43 adds the ordered QueueSort-through-PostBind framework over coherent revisioned snapshots, fail-closed plugins, hard sovereignty/resource/runtime/topology filters, weighted DRF, quota and priority ordering, atomic reserve/unreserve and CAS bind, permit waits, all-or-nothing gangs, multi-victim disruption-aware preemption, spread and accelerator-locality scoring, authoritative budget/ROI inputs, and starvation bounds. The 10-case deterministic gate exercises 256 adversarial multi-tenant histories with no bypass, double bind, oversubscription, reservation leak, or feasible-work starvation. | Preserve the framework contract while later controller, scale, federation, and release prompts compose it into their production profiles and auditable grant flows. |
| Node runtime | Partial | SAD-33 adds last-moment signed child-capability verification. SAD-35 additionally requires the verified persisted placement/runtime handoff at the actual process-driver start seam and proves close/reopen recovery in the live bridge gate. | Extend the same enforcement to container parity, NodeLease behavior, drift recovery, and target release/scale profiles. |
...
|---|---|---|---|
| Hard feasibility filters | Present | SAD-32's trust and connectivity predicates remain mandatory; SAD-43 adds tenant allowlists, lease/cordon, runtime class, quota, full resource vectors, placement-group topology, and coherent-snapshot checks before score. | Preserve deny-wins ordering and add later grant/audit profile evidence without weakening any predicate. |
| Pluggable scoring | Present | SAD-43 supplies explicit PreScore and normalized Score phases with weighted utilization, topology/locality, spread, consolidation, and authoritative budget/ROI components plus deterministic plugin descriptors and failure posture. | Preserve configuration validation and signal provenance at production-profile scale. |
| QueueSort | Present | Weighted DRF, guarantees, priority, age/starvation, gang identity, and stable work identity form a deterministic queue order. | Reprove bounds under release and scale profiles. |
| PreFilter and PostFilter | Present | The extension lifecycle includes ordered PreFilter, Filter, and PostFilter phases with structured fail-closed outcomes. | Preserve phase ordering and bounded plugin execution in production composition. |
| Reserve and Unreserve | Present | Revision-checked reservations are atomic and every reject, expiry, plugin failure, panic, stale bind, and failed gang path unwinds through Unreserve. | Bind reservation evidence into later durable audit and recovery profiles. |
| Permit | Present | Permit supports approve, bounded wait, reject, expiry, and gang admission without leaking reservations. | Compose later asynchronous policy adapters without changing fail-closed timeout semantics. |
| PreBind, Bind, and PostBind | Present | The explicit transactional lifecycle binds against the coherent revision with stable workload generation/replica identity and rejects stale or duplicate binds. | Publish later production-host audit linkage and target-load evidence. |
| Coherent snapshot scheduling | Present | A scheduling cycle clones one revisioned state, evaluates against it, and commits reservation/bind mutations only through revision-checked atomic state replacement. | Reprove the contract against later durable-store and HA scale profiles. |
| Deterministic tie-breaking | Present | Stable workload UID, generation, replica, and node identity seed the deterministic tie-break; repeat and adversarial histories prove replica-stable outcomes. | Preserve this identity contract across serialization upgrades. |
| Quotas | Present | SAD-40 defines ResourceQuota; SAD-43 enforces weighted hard resource and spend limits before reservation and recomputes usage invariants after every committed transition. | Extend quota observability and profile-scale evidence. |
| Dominant-resource fairness | Present | Weighted DRF spans CPU, memory, ephemeral storage, GPU, NPU, accelerator memory/slots, spend, and configured extended resources, with guarantees and a declared starvation bound. | Measure fairness and latency under release and scale profiles. |
| Priority and preemption | Present | PriorityClass and disruption resources from SAD-40 feed deterministic multi-victim preemption that never relaxes trust filters, evicts equal/higher priority, or exceeds protection/disruption constraints; failed attempts roll back. | Reprove under controller failover and target churn. |
| Gang scheduling | Present | Placement groups use minimum-member, all-or-nothing reserve/permit/bind behavior with topology constraints and failure-safe release. | Reprove at scale and across controller recovery. |
| Topology and accelerator locality | Present | SAD-43 reuses only suitable weighted-interconnect concepts and adds explicit accelerator topology/locality, placement spread, and cost-aware selection while leaving AOG request batching and KV behavior outside Saddle. | Extend hardware discovery and profile-specific topology fixtures. |
...
| Authorized AOG dispatch | Present | SAD-35 composes configured gateway provider calls, toolproxy executor calls, and Saddle control effects through tenant/runtime-class `ActionSessionRegistry` sessions derived from the verified runtime handoff. Exact lineage, availability, replay, budget, receipt, and current revocation checks precede the private effect. Standalone compatibility constructors remain for package-level use, while managed Saddle composition supplies the registry. | Preserve mandatory registry composition in managed runtimes and extend conformance across later controller, scale, federation, and release profiles. |
...
| Fail-closed behavior | Present | SAD-33 and SAD-34 prove the adversarial node/action boundaries. SAD-35 adds verified persisted handoff consumption and proves that cross-tenant theft, tenant-local authority loss, and signed revocation deny model/tool/control effects while the unaffected tenant remains live; recovery restores only valid authority. | Reprove these invariants under the larger partition, offline, federation, scale, and release-profile gates. |
...
| SC-06 | Weighted dominant-resource fairness and starvation bounds | Complete at deterministic gate | SAD-43 proves weighted DRF, guarantees, priority interaction, and feasible-work selection within the declared bound across adversarial tenant histories. |
| SC-07 | Priority, preemption, disruption budgets, and safe rollback | Complete at deterministic gate | SAD-43 proves deterministic multi-victim preemption respects trust, priority, protection/disruption limits, capacity, and rollback invariants. |
| SC-08 | Atomic gang scheduling and placement-group semantics | Complete at deterministic gate | SAD-43 proves all-or-nothing placement-group reserve, permit approve/wait/reject/expiry, bind, and leak-free release behavior. |
...
| SC-12 | End-to-end WSF identity and grant continuity | Present | SAD-31 through SAD-35 wire real admission, scheduling, signed child capability, persisted typed runtime handoff, node process start, and managed AOG model/tool/control effects. The live two-tenant gate proves restart, authority-loss isolation, revocation, and receipt continuity; later profiles must reprove it at scale. |
| SC-13 | Authorized AOG gateway, tool mint, and execution | Partial | SAD-35 routes configured gateway provider and toolproxy executor effects through the verified runtime-to-action path with proof before effect. Production credential-mint composition and later release/scale profile coverage remain open. |
...
4. **Create the authority bridge.** Complete: SAD-30 through SAD-34 established the typed grant chain, real admission, hard placement, child-capability lifecycle, and proof-before-effect action gate. SAD-35 completed the persisted placement/runtime handoff, managed gateway/toolproxy/control composition, and live two-tenant outage/restart/revocation/receipt proof.
5. **Expand the resource and scheduler model.** Complete at the SAD-40/SAD-43 deterministic gates: the six scheduling resources and complete queue-to-post-bind lifecycle, DRF, quota, topology, disruption-aware preemption, gangs, reservations, and atomic bind are implemented. Later profiles must compose and reprove them at target scale.
6. **Bind controllers and runtimes to grants.** Finish resource reconcilers, lease/drain behavior, and container-driver parity with SAD-35's last-moment persisted RuntimeGrant enforcement for process execution.
