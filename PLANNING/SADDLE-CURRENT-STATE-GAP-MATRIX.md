...
| Saddle identity | Partial | SAD-20 moved the canonical 12-package set and the `saddled`, `saddle-noded`, and `saddlectl` binaries with no old Cargo aliases. Protocol, trust, persistence, deployment, and documentation identities remain for SAD-21 through SAD-23. | Complete the bounded runtime/protocol migration, versioned legacy-state conversion, and active-name eradication gates. |
...
| Wire and transport security | Partial | `aog-wire` and the WSF fabric provide framed messages, identity-bearing transport, and mTLS-related enforcement. Saddle protocol naming and full live trust evidence are absent. | Rename to `saddle-wire`; enforce Saddle SPIFFE identities and reproduce live trust-boundary tests. |
| API and admission | Partial | `aog-apiserver` verifies WSF tokens and builds `VerifiedRequestContext`, with audit recovery. `admit_system` and `Principal::system` remain an unacceptable production shortcut for the target contract. | Rename; remove or strictly test-confine the system bypass; require explicit workload identity and grants on every production path. |
| WSF-AOG bridge | Missing | Reusable WSF principals and canonical resources exist, but there is no explicit `saddle-bridge` component or the required non-deserializable grant pipeline. | Implement the bridge and typed VerifiedSaddleRequest, AdmissionGrant, PlacementGrant, RuntimeGrant, and ActionGrant flow. |
...
| Scheduler | Partial | `aog-scheduler` has readiness, ring, attestation, and capacity filters; utilization and spread scorers; traceable signals; deterministic selection; and limited preemption. It is a filter-score selector, not the required scheduling framework. | Implement the full extension cycle, fairness, reservations, permits, gang semantics, quotas, topology, disruption policy, and atomic binding. |
...
|---|---|---|---|
| Hard feasibility filters | Present | Readiness, trust-ring, attestation, and capacity filters exist. | Add quota, runtime-class, topology, placement-group, disruption, lease, and grant-aware filtering. |
...
| Typed grant progression | Missing | No required five-stage Saddle request/grant pipeline exists. | Introduce non-deserializable verified types so downstream components cannot manufacture authority. |
...
| SC-12 | End-to-end WSF identity and grant continuity | Partial | WSF admission exists, but there is no saddle-bridge grant chain and a system bypass remains. |
...
3. **Complete the Saddle rename.** SAD-20 completed the 12-package and three-binary cutover. SAD-21 through SAD-23 still own protocol, environment, deployment, API-group, finalizer, label, SPIFFE, legacy-state, and documentation identity. Leave Loom only in explicit historical migration notes and bounded migration fixtures.
