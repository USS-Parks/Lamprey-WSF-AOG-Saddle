...
| Saddle identity | Partial | SAD-20 moved the canonical 12-package set and three binaries; SAD-21 moved active runtime identities with no aliases; SAD-22 added deterministic inspect/dry-run/apply/verify/rollback conversion that preserves authority, versions, the independent receipt chain, and exact rollback. The final repository-wide eradication gate remains. | Complete SAD-23 active-name eradication without removing historical provenance or the bounded migration surface. |
...
3. **Complete the Saddle rename.** SAD-20 completed the 12-package and three-binary cutover; SAD-21 completed the active runtime, protocol, trust, persistence, deployment, API-group, finalizer, label, SPIFFE, fixture, CI, and operator-document cutover; SAD-22 completed versioned legacy-state migration with exact rollback and authority/receipt/version preservation. SAD-23 owns the final active-name eradication gate. Leave Loom only in explicit historical migration notes and bounded migration fixtures.
