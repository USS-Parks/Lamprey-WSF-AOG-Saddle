...
| WSF-AOG bridge | Partial | SAD-30 froze the non-deserializable request/admission/placement/runtime/action contracts. SAD-31 consumes `VerifiedSaddleRequest` and `AdmissionGrant` through real CRUD and proves missing, stale, spoofed, out-of-scope, replayed, and cross-tenant authority fails closed. | Wire placement, node runtime, per-action enforcement, and the live two-tenant path in SAD-32 through SAD-35. |
...
4. **Create the authority bridge.** SAD-30 completed the explicit typed contract and compatibility/property gate; SAD-31 completed real WSF-authenticated admission plus durable grant/audit linkage. SAD-32 through SAD-35 must wire scheduling, controller identities, runtime/action enforcement, the live two-tenant gate, and remove production system bypasses.
