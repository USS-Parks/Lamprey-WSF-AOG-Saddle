...
**J-10b — Independent evidence-tool closure.** After J-10, run the local three-layer evidence package and close every independent probe that is now available locally. Scope: `cargo audit`, `cargo deny`, repository-wide `pytest` collection, `ruff`, `bandit`, `pip-audit`, `gitleaks`, `detect-secrets`, `tokei`, `scc`, `radon`, and `hadolint` if Dockerfile work creates a Docker surface. Outcome: all installed independent probes are PASS, or any remaining FAIL has a signed deferral with owner, reason, and post-RC lane. This is not a new feature lane; it is the quality gate proving the local scan is more than a GitDoctor mimic.
...
### W9 — Re-scan + Close (P1)
1. **J-14:** Re-run GitDoctor against the post-W1..W7 + W10 HEAD, including J-10b and all adapter completion sessions J-18..J-26. Capture all screenshots into `mai/test-evidence/dougherty-rescan/`. Target: overall ≥75/100, Code Quality ≥70, Testing ≥60, zero HIGH security findings, zero HIGH project findings.
...
```
  ...
  J-05 ─ J-06 ─ J-07                       W3 Adapter completion matrix starts
         ...
         │
  J-09 ─ J-10 ─ J-10b                       W5 Test quality + independent evidence closure
         │
  ...
  J-XX  ────── (W8 Evidence pack: J-XX — written EARLY, refreshed at end)
         │
         v
  J-14  Rescan  (depends on J-01..J-13, J-10b, AND J-16..J-26)
         │
         v
  J-15  Send + RC-10 prep
```

**Critical path (longest sequential chain):** J-02 → J-03 → J-04 → J-05 → J-06 → J-07 → J-18 → J-19 → J-20 → J-21 → J-22 → J-23 → J-24 → J-25 → J-26 → J-16 → J-17 → J-10b → J-14 → J-15. Roughly 20 sessions of serial work, with safe parallelism possible across independent adapter sessions once J-05 defines the shared matrix.
...
| W5 | Every `test_adapter.py` has ≥30 assertions. CI gate fails when any new `test_*.py` has fewer than 3 assertions. GitDoctor TST-004 cleared in re-scan. J-10b evidence package shows all installed independent probes PASS, except signed deferrals. |
