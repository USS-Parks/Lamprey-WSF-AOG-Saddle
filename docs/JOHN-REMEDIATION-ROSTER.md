...
| J-10b| Independent evidence-tool closure                          | W5         | N + reports     | J-10       | M      |
...
| J-14| Re-run GitDoctor, capture evidence                          | W9         | evidence only  | J-01..J-13, J-10b, J-16..J-26 | XS |
...
### Session J-10b: Independent evidence-tool closure

**Workstream:** W5 (Independent Evidence Gate)
**Depends on:** J-10
**Blocks:** J-14
**Files in play:** `mai/tools/local_gitdoctor_evidence.py` (edit only if runner needs a bug fix), failing source files discovered by probes, `mai/docs/LOCAL-GITDOCTOR-EVIDENCE.md`, `mai/docs/LOCAL-GITDOCTOR-EVIDENCE.json`, optional `mai/docs/INDEPENDENT-EVIDENCE-DEFERRALS.md`
**Expected line delta:** unknown; prefer targeted edits. Deferral doc ~40-80 lines only if needed.

#### Context Brief

After the local audit tool install, the independent evidence layer is real. Latest known state before this roster amendment:

- PASS: adversarial fixtures, `cargo check`, `cargo clippy`, `cargo test`, `tokei`, `scc`, `radon`.
- FAIL: `cargo audit`, `cargo deny`, repository-wide `pytest`, `ruff`, `bandit`, `pip-audit`, `gitleaks`, `detect-secrets`.
- SKIP: `hadolint` if it is not installed.

This session exists because J-14 must not only re-run the original GitDoctor-style checks; it must prove the local evidence package is not merely mimicking GitDoctor. Close independent probe failures or document signed deferrals.

#### Prompt

SESSION J-10b: Independent evidence-tool closure

CONTEXT: The local evidence package now runs mature independent tools in addition to mapped GitDoctor-style checks. J-14 is blocked until all installed independent probes PASS or any remaining FAIL has a signed deferral.
...
2. Parse `docs/LOCAL-GITDOCTOR-EVIDENCE.json`. Create a short table of FAIL/SKIP probes before editing anything.

IMPLEMENT:
1. `cargo audit`: fix vulnerable Rust dependency versions if possible with minimal version bumps. If a RustSec advisory has no compatible fix, document it in `docs/INDEPENDENT-EVIDENCE-DEFERRALS.md` with advisory ID, affected crate, why it is not exploitable or why it is deferred, and owner.
2. `cargo deny`: add or update `deny.toml` only if the policy file is missing or stale. Do not blindly allow advisories. The goal is a real policy pass, not a suppression blanket.
3. `pytest`: fix repository-wide collection/import errors. Current known failures are app smoke/integration tests and `mai-sdk-python/tests/*` resolving as `tests.test_*`. Prefer packaging/testpath fixes over deleting tests.
4. `ruff`: fix lint findings that are mechanical and low-risk. If rule scope is too broad for this lane, add a narrow `pyproject.toml` rule selection with rationale, not a broad ignore-all.
5. `bandit`: fix true positives. For false positives, add targeted `# nosec` comments only with a short reason.
6. `pip-audit`: update Python lock files for fixable vulnerable packages. If no patched release exists, document the advisory in the deferral doc.
7. `gitleaks` and `detect-secrets`: remove real secrets. For synthetic fixtures, generated hashes, or intentionally public placeholders, add a committed baseline/config with exact-path allowlist and rationale.
8. `hadolint`: if Dockerfile exists and hadolint is installed, make Dockerfile pass or document why the rule is not applicable. If hadolint is not installed, list it as an environment skip, not a project pass.
9. Do not chase mapped code-quality findings in this session unless they are the direct cause of an independent probe failure.

VERIFY:
- Re-run the three commands from START.
- `docs/LOCAL-GITDOCTOR-EVIDENCE.json` shows:
  - adversarial fixture: PASS.
  - every installed independent probe: PASS, except documented deferrals.
  - SKIPPED only for tools absent from the host, not for project failures.
- `docs/INDEPENDENT-EVIDENCE-DEFERRALS.md`, if created, has one section per remaining non-pass probe with owner, reason, expiry/revisit session, and exact command output reference.
- Subagent verification required if 3+ files are touched.

COMMIT: one commit, message `J-10b: close independent evidence tool failures before GitDoctor rescan`. Footer per global rule.

#### Acceptance Criteria

- `python tools/local_gitdoctor_evidence.py --root . --output docs/LOCAL-GITDOCTOR-EVIDENCE.md --json-output docs/LOCAL-GITDOCTOR-EVIDENCE.json` completes with no unexpected runner crash.
- All installed independent probes are PASS or explicitly listed in `INDEPENDENT-EVIDENCE-DEFERRALS.md`.
- No broad suppressions were added without rationale.
- J-14 is unblocked only after this status is true.
...
**Workstream:** W9
**Depends on:** J-01..J-13, J-10b, J-16..J-17, and J-18..J-26 all committed and pushed to origin/main
**Blocks:** J-15
...
CONTEXT: J-01..J-13, J-10b, J-16..J-17, and the full W3 adapter matrix J-18..J-26 are committed. Re-run the scan and capture results.
...
| J-10b   | YES | maybe (deferral doc/config baselines) | staged if new | mixed |
...
- [ ] J-10b committed (independent evidence probes PASS or signed deferrals).
...
- [ ] J-14 committed (re-scan evidence — runs AFTER J-10b, J-13, J-17, and J-26).
