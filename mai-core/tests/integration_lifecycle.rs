...
use mai_core::scheduler::{RequestType, RequestPayload, ChatMessage, SchedulingStrategy};
...
async fn test_full_request_lifecycle() {
    ...
    assert!(routed_70b.is_err(), "Should fail: only 70b adapter is unhealthy");
    ...
    assert!(power.check_auto_demotion().is_none(), "Just reset: should not demote");
...
async fn test_hotswap_adapter_replacement() {
    let scheduler = Arc::new(RwLock::new(Scheduler::new(SchedulerConfig::default()).unwrap()));
    ...
    match result {
        SwapResult::Success { drained_requests, completion_time } => {
