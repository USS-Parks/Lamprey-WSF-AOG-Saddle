...
use saddlectl::{Client, ClientError};
...
async fn spawn(dir: &str, budget: Option<Budget>) -> (SocketAddr, String) {
    ...
    tokio::spawn(async move { saddle_apiserver::serve(listener, state).await });
...
async fn apply_then_get_roundtrips() {
    let (addr, token) = spawn("saddlectl-roundtrip", None).await;
...
async fn over_budget_apply_is_rejected_client_visibly() {
    ...
    let (addr, token) = spawn("saddlectl-budget", Some(broke)).await;
    let client = Client::new(format!("http://{addr}"), token);
