//! `saddlectl` — the Saddle control-plane CLI client (kernel subset).
//!
//! A thin HTTP client over `saddle-apiserver`'s typed CRUD surface. Every request
...
/// An `saddle-apiserver` HTTP client.
pub struct Client {
    base: String,
    token: String,
    http: reqwest::Client,
