...
//! tool receipt). Both services are then brought under Saddle management —
...
async fn provision(c: &Client, addr: &str, tok: &str) -> (String, String) {
    ...
    bao(
        ...
        Some(json!({"token_policies":"default,saddle-x3","token_ttl":"15m"})),
    )
    .await;
...
async fn tool_calls_and_metering_continue_across_the_cutover_receipts_unbroken() {
    ...
    let anchor = RustCryptoMlDsa87::generate("saddle-x3-anchor").unwrap();
    ...
    // ── Bring both services under Saddle management.
    let estate = AppState::bootstrap(
        1,
        fresh_dir("saddle-x3-estate"),
    ...
    client
        .ensure_created(ResourceObject::Node(Resource::new(
            ...
            NodeSpec {
                ...
                attestation: saddle_estate::AttestationProfile::default(),
                capacity: saddle_estate::Capacity::default(),
            },
        )))
        .await
        .unwrap();
