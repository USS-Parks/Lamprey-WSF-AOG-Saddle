//! `saddle-scheduler` — Saddle's attested placement engine (Phase S).
//!
//! Saddle places AOG [`Workload`](saddle_estate::Workload)s onto estate
//! [`Node`](saddle_estate::Node)s. This crate is the decision engine that chooses
...
//! [`Node`](saddle_estate::Node); absence is fail-closed, never fabricated.** A
...
//! (S8). Writing the [`Placement`](saddle_estate::Placement) and minting the
