//! `saddle-estate` — the typed resource model for the Saddle orchestration control
//! plane (M3, addendum A1.5). These are Saddle's "CRDs": versioned,
...
//! no ad-hoc structs"). The store (`saddle-store`) persists these as desired
//! state; the apiserver admission chain (`saddle-apiserver`) validates, seals,
//! and receipts them.
//!
...
pub struct ObjectMeta {
    ...
    #[serde(default)]
    pub generation: u64,
    /// Store revision (set by `saddle-store`; `0` until first admitted).
    #[serde(default)]
...
/// A typed Saddle resource: `type_meta` + `metadata` + `spec` + optional `status`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
macro_rules! resource_objects {
    ($($kind:ident),+ $(,)?) => {
        /// Any Saddle resource, tagged by its kind — the unit `saddle-store` persists
        /// and `saddle-apiserver` admits.
        ...
        impl ResourceObject {
            ...
            /// (`saddle-apiserver`) stamps uid / generation / token_ref / receipt_ref here.
