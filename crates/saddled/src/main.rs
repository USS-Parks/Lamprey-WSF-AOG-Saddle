//! `saddled` — the Saddle control-plane node daemon.
//!
//! Reads its identity + listen address from the environment, starts a
//! [`Daemon`](saddled::Daemon) (a `RaftNode` on the `saddle-wire` transport), and serves
...
use saddle_wire::tls::{TlsListener, TlsPeer};
use saddled::{Config, Daemon, DaemonError};
