//! `saddled` — the minimal Saddle control-plane node daemon (Phase V).
//!
//! The control plane's over-the-wire Raft transport lives in `saddle-wire`; this
...
use saddle_store::raft::types::NodeId;
use saddle_wire::WireNetwork;
use saddle_wire::tls::{NodeIdentityContract, NodeTls};

pub use api::{ChangeMembershipRequest, GetRequest, InitializeRequest, LeaderStatus, Member};
pub use client::{Client, ClientError};
pub use saddle_store::raft::types::RaftResponse;
pub use saddle_store::{Op, Precondition, Versioned};
...
pub enum DaemonError {
    ...
    Config(String),
    #[error("node: {0}")]
    Node(#[from] saddle_store::raft::NodeError),
    #[error("io: {0}")]
...
pub struct Config {
    ...
    /// `saddle-apiserver` CRUD surface; when `None`, only the wire + admin
...
/// A running control-plane node daemon: a [`RaftNode`] on the `saddle-wire` transport
...
impl Daemon {
    ...
    pub async fn start(config: Config) -> Result<Self, DaemonError> {
        ...
        // saddle-apiserver CRUD over this very node (the `from_raft` seam), fail-closed.
    ...
    /// The combined axum app: the `saddle-wire` Raft peer endpoints (`/raft/*`) merged
    /// with the admin API (`/admin/*`, `/healthz`).
    pub fn app(&self) -> Router {
        let wire = if self.secure_transport {
            saddle_wire::secure_router(Arc::clone(&self.node))
        } else {
            saddle_wire::router(Arc::clone(&self.node))
        };
        let mut app = wire.merge(admin::router(
            Arc::clone(&self.node),
            self.state
                .as_ref()
                .map(saddle_apiserver::AppState::authenticator),
        ...
        if let Some(state) = &self.state {
            app = app.merge(saddle_apiserver::api_router(state.clone()));
        }
        app
    }
