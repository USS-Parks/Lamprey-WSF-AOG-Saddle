...
//! That is the kernel default the `saddle-apiserver` seal explicitly flags as the
//! Phase-W OpenBao seam. This module is that seam for `saddled`: at startup a node
...
/// The trust material a Saddle node needs to serve the authenticated CRUD surface.
...
const SEAL_KEY_ID: &str = "saddle-apiserver-cp";
...
mod tests {
    ...
    fn node_tls_record() -> serde_json::Value {
        let dir = std::env::temp_dir().join("saddled-node-tls-record");
