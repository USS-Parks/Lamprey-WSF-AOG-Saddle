//! C2 control-plane gate: three `saddled` nodes converge over real mutual TLS.
...
use saddle_store::Op;
use saddle_store::raft::types::TypeConfig;
use saddle_wire::tls::{NodeIdentityContract, NodeTls, TlsListener, TlsPeer};
use saddled::{Client, Config, Daemon, Member, NodeTlsProvisioning, Precondition};
...
fn scratch() -> PathBuf {
    let dir = std::env::temp_dir().join("saddled-c2-mtls");
...
async fn three_node_raft_requires_mutual_tls_and_exact_sender_identity() {
    ...
    let response = clients[0]
        ...
        .await
        .unwrap();
    assert!(matches!(response, saddled::RaftResponse::Applied { .. }));
