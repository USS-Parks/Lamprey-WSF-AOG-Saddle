//! `saddled` sources its trust anchor **and** field-seal material
...
use saddled::{Config, Daemon, OpenBaoTrust};
use serde_json::json;
...
#[tokio::test]
async fn saddled_trust_from_live_openbao() {
    let Some(addr) = openbao_addr() else {
        eprintln!("SKIP saddled_trust_from_live_openbao: WSF_OPENBAO_ADDR unset (live gate)");
        return;
    };
    ...
    // Start saddled sourcing its trust from OpenBao (no env anchor).
