//! Three `saddled` daemons form a control plane and replicate a write,
//! driven entirely through the admin API over the `saddle-wire` transport (not the
...
use saddled::{Client, Config, Daemon, Member, Op, Precondition};
...
async fn three_saddled_daemons_form_consensus_through_the_admin_api() {
    ...
    // Start each saddled daemon and serve its combined (raft + admin) app.
    ...
    assert!(
        matches!(response, saddled::RaftResponse::Applied { .. }),
