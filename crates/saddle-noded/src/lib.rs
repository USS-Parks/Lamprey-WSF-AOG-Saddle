//! `saddle-noded` — the runnable Saddle edge/worker node daemon (Phase V).
//!
//! The control plane is runnable (`saddled`); this crate makes the **edge** runnable.
//! `saddle-noded` registers its `Node` (attestation profile + capacity) with the
//! control plane over the wire — through the admin API, reusing `saddled::Client`
...
//!
//! Identity-verified registration (the `saddle-node` `Registrar` / anchor-signed leaf)
...
use saddle_estate::{AttestationProfile, Capacity, Node, NodeSpec, Resource};
use saddle_node::heartbeat::heartbeat;
use saddled::{Client, Op, Precondition};
...
pub enum NodedError {
    ...
    ControlPlane(#[from] saddled::ClientError),
    #[error("encode: {0}")]
...
pub struct NodeConfig {
    ...
    /// Base URL of a control-plane `saddled` (e.g. `http://cp1:4600`).
...
impl NodeAgent {
    /// Build the agent from `config`: an `saddled` client to the control plane and the
