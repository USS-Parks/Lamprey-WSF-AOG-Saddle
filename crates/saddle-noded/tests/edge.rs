//! Edge `saddle-noded` daemons register with the control plane and stay
//! live. A bootstrapped `saddled` control-plane node plus three edge daemons over
...
use saddle_estate::{Capacity, Node};
use saddle_node::heartbeat::is_stale;
use saddle_noded::{NodeAgent, NodeConfig};
use saddled::{Client, Config, Daemon, Member};
...
async fn edge_daemons_register_and_heartbeat_into_the_control_plane() {
    ...
    // Edge nodes: start three saddle-noded daemons pointed at the control plane.
