...
async fn missing_token_is_unauthorized() {
    let (app, _tok) = authed_app("saddle-apiserver-k6-missing").await;
...
async fn health_probe_is_open() {
    let (app, _tok) = authed_app("saddle-apiserver-k6-health").await;
...
async fn wrong_anchor_is_unauthorized() {
    ...
    let signer_a = anchor();
    let app = app_anchored("saddle-apiserver-k6-wronganchor", &signer_a, None).await;
...
async fn expired_token_is_unauthorized() {
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-k6-expired", &signer, None).await;
...
async fn over_budget_token_is_payment_required() {
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-k6-budget", &signer, None).await;
...
async fn valid_token_is_admitted() {
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-k6-valid", &signer, None).await;
