...
async fn tenant_requiring_unheld_scope_is_forbidden() {
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-k7-hipaa", &signer, None).await;
...
async fn deny_wins_across_regimes() {
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-k7-denywins", &signer, None).await;
...
async fn classification_over_authority_is_forbidden() {
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-k7-class", &signer, None).await;
...
async fn compliant_tenant_is_admitted() {
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-k7-ok", &signer, None).await;
...
async fn non_compliance_kind_is_admitted() {
    ...
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-k7-noop", &signer, None).await;
    let tok = header_for(&mint_with(&signer, |t| {
        t.compliance_scopes = vec![];
    }));
    let (status, _) = send(
