...
async fn crud_roundtrip() {
    let (app, tok) = authed_app("saddle-apiserver-k5-crud").await;
...
async fn reads_are_tenant_scoped_and_estate_pagination_is_explicit() {
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-xtenant-read", &signer, None).await;
...
async fn revocation_targets_require_exact_tenant_or_estate_capability() {
    let signer = anchor();
    let app = app_anchored("saddle-apiserver-revocation-scope", &signer, None).await;
...
async fn duplicate_create_conflicts() {
    let (app, tok) = authed_app("saddle-apiserver-k5-conflict").await;
    let t = Some(tok.as_str());
...
async fn update_missing_is_not_found() {
    let (app, tok) = authed_app("saddle-apiserver-k5-updatemiss").await;
    let (status, _) = send(
        &app,
        "PUT",
...
async fn unknown_kind_is_bad_request() {
    let (app, tok) = authed_app("saddle-apiserver-k5-unknownkind").await;
