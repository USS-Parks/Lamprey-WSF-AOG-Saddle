...
async fn one_receipt_per_mutation_and_pack_verifies_offhost() {
    let (app, state, tok) = authed_app_state("saddle-apiserver-k9").await;
    ...
    let (_, other_state, _) = authed_app_state("saddle-apiserver-k9-otherkey").await;
    assert!(!verify_pack(
        &pack,
        &MlDsa87Verifier,
...
async fn rejected_mutation_emits_no_receipt() {
    let (app, state, tok) = authed_app_state("saddle-apiserver-k9-reject").await;
    let t = Some(tok.as_str());
...
async fn pending_intent_survives_crash_and_recovers_as_verifiable_evidence() {
    let dir = fresh_dir("saddle-apiserver-audit-crash-window");
