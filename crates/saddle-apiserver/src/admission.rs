        ...
        Self {
            request_principal: WsfPrincipal::establish(
                AuthenticatedFacts {
                    ...
                    service_identity: Some("saddle-controller".to_owned()),
...
/// The sole writer to `saddle-store`. Its `raft` handle is private; see the module
...
impl Admission {
    ...
    fn authorize_revocation(
        ...
        req: &AdmissionRequest,
        principal: &Principal,
        target: &saddle_estate::RevocationTarget,
    ) -> Result<(), ApiError> {
        if let Some(tenant) = principal.tenant() {
            if matches!(target, saddle_estate::RevocationTarget::Ring(_)) {
                return Err(ApiError::Forbidden(
                    "tenant principal cannot create an estate-wide ring revocation".to_owned(),
                ));
            }
            if let saddle_estate::RevocationTarget::Tenant(target_tenant) = target
    ...
    fn deliver_receipt(&self, receipt: serde_json::Value) {
        ...
        let _ = self
            ...
            .ingest("saddle-apiserver", receipt);
    }
