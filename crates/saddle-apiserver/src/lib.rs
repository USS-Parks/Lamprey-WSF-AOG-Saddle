//! `saddle-apiserver` — the Saddle control-plane API server.
//!
//! A typed CRUD surface over the estate kinds (`saddle-estate`) backed by the
//! consensus store (`saddle-store`). Two invariants this crate exists to hold:
...
use saddle_store::raft::{NodeError, RaftNode};
...
impl AppState {
    ...
    /// seam. It lets the wire-transport control-plane daemon (`saddled`) serve the
    ...
    #[must_use]
    pub fn informer(&self, prefix: impl Into<String>) -> saddle_store::raft::watch::Informer {
...
/// its own health/liveness surface — the wire-transport daemon `saddled` —
