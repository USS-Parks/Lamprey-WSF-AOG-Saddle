//! In-process bar checks that run against the real `saddle-store` Raft state machine
//! and the real `saddle-controller` reconcile runtime. Each returns `Ok(detail)` on
...
use saddle_controller::{Action, AlwaysLeader, Controller, ReconcileError, Reconciler};
use saddle_store::raft::types::RaftResponse;
use saddle_store::raft::{Cluster, RaftNode};
...
pub async fn idempotent_reconcile(histories: usize, seed: u64) -> Result<String, String> {
    ...
    let late_key = "Saddle/k6";
    // The one authoritative end state after base + late (put k0=v2, del k1, put k6).
    let expected: BTreeMap<String, Option<String>> = [
        ("Saddle/k0".to_owned(), Some("v2".to_owned())),
        ("Saddle/k1".to_owned(), None),
        ("Saddle/k2".to_owned(), Some("v1".to_owned())),
        ("Saddle/k3".to_owned(), Some("v1".to_owned())),
        ("Saddle/k4".to_owned(), Some("v1".to_owned())),
        ("Saddle/k5".to_owned(), Some("v1".to_owned())),
        ("Saddle/k6".to_owned(), Some("v1".to_owned())),
    ]
    .into_iter()
    .collect();
    ...
    for i in 0..histories {
        ...
        put(&node, "Saddle/k0", "v2").await?;
        delete(&node, "Saddle/k1").await?;
        put(&node, late_key, "v1").await?;

        // 4. Deliver the affected keys under a randomized mode: reorder + duplicate.
        let mut order: Vec<&str> = vec!["Saddle/k0", "Saddle/k1", late_key];
        ...
        // so the Saddle end state is untouched) — the next poll lags and re-lists.
        if prng.below(50) == 0 {
            overflow_runs += 1;
            for n in 0..80u32 {
...
/// real `saddle-controller` reconcile runtime at the aggressive-profile scale.
