...
//!   operator's divergence report: what Saddle would have done.
...
fn replay(
    ...
    legacy: &saddle_node::driver::WorkloadHandle,
) -> saddle_node::driver::WorkloadHandle {
...
fn an_identical_estate_differs_across_modes_and_shadow_never_disrupts() {
    ...
    assert_eq!(
        ...
        "report-only surfaces exactly what Saddle would have done"
    );
