//! A 3-node Saddle control plane forms consensus over the real
//! `saddle-wire` HTTP transport (not the in-process `Cluster`): a leader is elected,
...
use saddle_wire::{WireNetwork, router};
