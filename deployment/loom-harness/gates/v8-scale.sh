...
write_to() { # svc key value-json-array — bounded retry: an election window on a
  ...
  # This explicit development harness has no node mTLS, so followers correctly
  # refuse internal forwarding. Resolve the current leader after a miss and
  # direct the client write there; production forwarding remains mTLS-only.
  wt_svc="$1"
  wt_n=0
  while :; do
